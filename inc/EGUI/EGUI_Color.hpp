...
namespace egui{
	class Color_RGBA{
	public:
		constexpr void setColors(const uint8_t R, const uint8_t G, const uint8_t B, double A){ 
			_r = R;
			_g = G; 
			_b = B;
			_a = std::clamp(A, 0.0, 1.0);
		}

		constexpr uint8_t& R(){ return _r; }
		constexpr uint8_t& G(){ return _g; }
		constexpr uint8_t& B(){ return _b; }
		constexpr double& A(){ return _a; }

		Color_RGBA(){}
		Color_RGBA(const uint8_t R, const uint8_t G, const uint8_t B, double A=1.0) : _r(R), _g(G), _b(B){
			_a = std::clamp(A, 0.0, 1.0);
		}
		~Color_RGBA(){};
	private:
